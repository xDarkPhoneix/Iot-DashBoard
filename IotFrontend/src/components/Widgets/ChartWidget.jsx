...
const ChartWidget = ({ widget, onRemove }) => {
    ...
    datasets: [
      {
        label: widget.title || 'Device Data',
        data: history,
        borderColor: widget.config && widget.config.color ? widget.config.color : '#3B82F6',
        backgroundColor:
          widget.config && widget.config.chartType === 'line'
            ? `${widget.config.color || '#3B82F6'}20`
            : widget.config && widget.config.color ? widget.config.color : '#3B82F6',
        fill: widget.config && widget.config.chartType === 'line',
        tension: 0.4,
        borderWidth: 2,
        pointRadius: widget.config && widget.config.chartType === 'line' ? 3 : 0,
        pointHoverRadius: 6,
      },
    ],
  };
  ...
  const options = {
    ...
    plugins: {
      legend: {
        display: widget.config && widget.config.showLegend,
        position: 'top',
      },
      tooltip: {
        mode: 'index',
  ...
  const ChartComponent = widget.config && widget.config.chartType === 'bar' ? Bar : Line;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-all duration-200 group">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 mt-1">
              <span className="text-2xl font-bold text-gray-900 dark:text-white">
                {typeof currentValue === 'number' ? currentValue.toFixed(1) : currentValue.slice(-1)}
              </span>
              <TrendingUp className="w-4 h-4 text-green-500" />
            </div>
          </div>
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
        ...
        <span className="flex items-center space-x-1">
          <div
            className={`w-2 h-2 rounded-full ${device && device.status === 'online' ? 'bg-green-500' : 'bg-red-500'}`}
          />
          <span className="capitalize">{device ? device.status : 'offline'}</span>
        </span>
      </div>
    </div>
  );
